...
case class GameState(
  ...
  /** Explicitly ends the current player's turn.
    *
    * Toggles the active color to the opponent and increments the full-move number if the current player is Black. Must
    * be called manually by the orchestrator after applying a sequence of micro-moves.
    *
    * @return
    *   a new [[GameState]] with the active color flipped and full-move number updated
    */
  def endTurn(): GameState =
    ...
    copy(
      flags = flags.withActiveColor(nextColor),
